...
def test_intraday_long_stop_loss_closes_once_without_reentry():
    ...
    assert pd.to_datetime(long_pos.pairs[0]["平仓时间"]).strftime("%H%M") == "0934"
    assert "100BP止损" in long_pos.pairs[0]["事件序列"]
    assert len([x for x in long_pos.operates if x["op"] == "LO"]) == 1
...
def test_intraday_short_position_holds_until_1455():
    ...
    assert pd.to_datetime(short_pos.pairs[0]["开仓时间"]).strftime("%H%M") == "0933"
    assert pd.to_datetime(short_pos.pairs[0]["平仓时间"]).strftime("%H%M") == "1455"
