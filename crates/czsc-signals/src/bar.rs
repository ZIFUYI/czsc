...
fn latest_bar_hhmm(c: &CZSC) -> Option<String> {
    let bar = c.bars_raw.last()?;
    Some(format!("{:02}{:02}", bar.dt.hour(), bar.dt.minute()))
}

/// bar_intraday_direction_V260424：日内固定时点方向信号
///
/// 参数模板：`"{freq}_D1T{decision_time}_日内方向V260424"`
///
/// 信号逻辑：
/// 1. 仅在 `decision_time` 对应的基础周期 K 线结束时计算；
/// 2. 比较当前收盘价与当日首根 K 线开盘价；
/// 3. 当前收盘价不低于当日开盘价输出 `看多`，否则输出 `看空`；
/// 4. 非决策时点或数据不足输出 `其他`。
#[signal(
    category = "kline",
    name = "bar_intraday_direction_V260424",
    template = "{freq}_D1T{decision_time}_日内方向V260424",
    opcode = "BarIntradayDirectionV260424",
    param_kind = "BarIntradayDirectionV260424"
)]
pub fn bar_intraday_direction_v260424(
    c: &CZSC,
    params: &ParamView,
    _cache: &mut TaCache,
) -> Vec<Signal> {
    let decision_time = params.str("decision_time", "0935");
    let k1 = c.freq.to_string();
    let k2 = format!("D1T{}", decision_time);
    let k3 = "日内方向V260424";

    if c.bars_raw.is_empty() || latest_bar_hhmm(c).as_deref() != Some(decision_time) {
        return make_kline_signal_v1(&k1, &k2, k3, "其他");
    }

    let last_bar = c.bars_raw.last().expect("checked non-empty");
    let trade_date = last_bar.dt.date_naive();
    let Some(day_open) = c
        .bars_raw
        .iter()
        .rev()
        .take_while(|bar| bar.dt.date_naive() == trade_date)
        .last()
        .map(|bar| bar.open)
    else {
        return make_kline_signal_v1(&k1, &k2, k3, "其他");
    ...
    let v1 = if last_bar.close >= day_open {
        "看多"
    } else {
        "看空"
    };
...
/// bar_intraday_exit_V260424：日内固定时点平仓信号
///
/// 参数模板：`"{freq}_D1T{exit_time}_日内平仓V260424"`
///
/// 信号逻辑：
/// 1. 仅在 `exit_time` 对应的基础周期 K 线结束时输出 `平仓`；
/// 2. 其他时间输出 `其他`。
#[signal(
    category = "kline",
    name = "bar_intraday_exit_V260424",
    template = "{freq}_D1T{exit_time}_日内平仓V260424",
    opcode = "BarIntradayExitV260424",
    param_kind = "BarIntradayExitV260424"
)]
pub fn bar_intraday_exit_v260424(
    c: &CZSC,
    params: &ParamView,
    _cache: &mut TaCache,
) -> Vec<Signal> {
    let exit_time = params.str("exit_time", "1455");
    let k1 = c.freq.to_string();
    let k2 = format!("D1T{}", exit_time);
    let k3 = "日内平仓V260424";
    let v1 = if latest_bar_hhmm(c).as_deref() == Some(exit_time) {
        "平仓"
    } else {
        "其他"
    };
